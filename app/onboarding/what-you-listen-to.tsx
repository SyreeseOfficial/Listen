...
const OPTIONS = [
  { key: 'albums', icon: '💿', label: 'Albums, start to finish', sub: 'The full experience, as intended' },
  { key: 'playlists', icon: '🎶', label: 'Playlists and mixes', sub: 'Curated, varied, flowing' },
  { key: 'vinyl', icon: '📻', label: 'Vinyl / physical media', sub: 'Ritual, warmth, and intention' },
  { key: 'podcasts', icon: '🎙', label: 'Podcasts and spoken word', sub: 'Voices, ideas, and stories' },
];

export default function WhatYouListenToScreen() {
  ...
  async function next() {
    if (!selected) return;
    await updateProfile({ listeningStyle: selected });
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={styles.header}>
        <Text style={[styles.title, { color: colors.text }]}>What do you listen to?</Text>
        <Text style={[styles.sub, { color: colors.textSecondary }]}>Pick your main thing.</Text>
      </View>
