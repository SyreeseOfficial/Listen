...
const OPTIONS = [
  { key: 'casual', icon: '🌊', label: 'Casual', sub: 'Music fills the space around me' },
  { key: 'curious', icon: '🔭', label: 'Curious', sub: 'I want to hear more in every track' },
  { key: 'dedicated', icon: '🕯', label: 'Dedicated', sub: 'I set time aside to really listen' },
  { key: 'audiophile', icon: '🎛', label: 'Audiophile', sub: 'Equipment and sound quality obsess me' },
];

export default function ListenerTypeScreen() {
  ...
  async function next() {
    if (!selected) return;
    await updateProfile({ listenerType: selected });
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={styles.header}>
        <Text style={[styles.title, { color: colors.text }]}>How do you listen?</Text>
        <Text style={[styles.sub, { color: colors.textSecondary }]}>We'll personalize your experience.</Text>
      </View>
