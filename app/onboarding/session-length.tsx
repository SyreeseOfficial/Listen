...
const OPTIONS = [
  { key: 'short', icon: '⚡', label: 'Under 15 min', sub: 'A quick escape', minutes: 10 },
  { key: 'medium', icon: '🎵', label: '15 – 30 min', sub: 'The sweet spot', minutes: 20 },
  { key: 'long', icon: '🎧', label: '30 – 60 min', sub: 'A proper sit-down', minutes: 45 },
  { key: 'deep', icon: '🌙', label: 'Over an hour', sub: 'Deep dive, no interruptions', minutes: 75 },
];
...
export default function SessionLengthScreen() {
  ...
  async function next() {
    ...
    await updateProfile({ defaultSessionMinutes: opt.minutes });
    router.push('/onboarding/what-you-listen-to');
  }
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={styles.header}>
        <Text style={[styles.title, { color: colors.text }]}>How long is a session for you?</Text>
        <Text style={[styles.sub, { color: colors.textSecondary }]}>This becomes your default. You can always change it.</Text>
      </View>
